...
## Accessibility(접근성) (A11y) {#AccessibilityA11y}

React Hook Form은 네이티브 폼 벨리데이션을 지원하며, 여러분만의 규칙으로 입력을 벨리데이션 할 수 있게 해줘요. 대부분의 경우 우리는 커스텀 디자인과 레이아웃을 가진 폼을 만들어야 하는데, 이러한 부분이 접근성(A11y)을 보장하는 것은 우리의 책임이에요.

다음 코드 예제는 벨리데이션을 위해 의도된 대로 작동하지만, 접근성을 개선할 수 있어요.
...
다음 코드 예제는 개선된 버전으로서 다음과 같이 발전시켰습니다.[ARIA를 사용했어요](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA).
...
이 개선 이후 스크린리더는 다음과 같이 작동할거에요: _“Name, edit, invalid entry, This is required.”_
